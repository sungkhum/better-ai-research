...
/**
 * Fix broken separator/continuation footnotes generated by docx-js.
 * docx-js adds <w:footnoteRef/> and <w:pPr> to separator footnotes,
 * which Word rejects. This function unpacks the DOCX, rewrites the
 * separator footnotes with clean XML, and repacks.
 */
function fixFootnoteXml(docxPath) {
  ...
  try {
    ...
    // Read and fix footnotes.xml
    const fnPath = path.join(tmpDir, "word", "footnotes.xml");
    let xml = fs.readFileSync(fnPath, "utf-8");

    // Replace the separator footnote (id="-1") with a clean version
    xml = xml.replace(
      /<w:footnote w:type="separator" w:id="-1">[\s\S]*?<\/w:footnote>/,
      `<w:footnote w:type="separator" w:id="-1"><w:p><w:r><w:separator/></w:r></w:p></w:footnote>`
    );

    // Replace the continuation separator footnote (id="0") with a clean version
    xml = xml.replace(
    ...
    // Repack — must zip from inside the directory to get correct paths
...
async function buildDocument() {
  ...
  // Post-process: fix docx-js footnote XML bug.
  // docx-js adds <w:footnoteRef/> to separator/continuation footnotes,
  // which Word considers invalid. Unpack, fix, repack.
  if (footnoteCount > 0) {
    fixFootnoteXml(outputPath);
  }
